...
### 1.1 Why not MCP "sampling"?

MCP defines `sampling/createMessage`, which lets a server ask the client's LLM to run a completion — exactly our use case in spirit. We deliberately do **not** make it the primary path:

- **Clients don't support it.** Claude Code does not implement sampling as an MCP client today; it is a long-standing, still-open gap. Codex and several other coding agents are in the same position. A primary path that most target clients can't execute is not a design — it's a wish.
- **It's being deprecated.** Sampling is marked deprecated as of MCP protocol revision **2026-07-28 (SEP-2577)**. Official guidance: new implementations should not adopt it and should integrate directly with provider APIs. Building our core integration on a deprecating primitive is a liability.

Conclusion: treat sampling as an **optional, best-effort accelerator** (Tier 2), gated behind capability detection, never as a requirement.
...
Tier 2  MCP SAMPLING (optional accelerator, off by default)
        ...
        initialize AND the operator opted in. Lets the server run autonomous
        background organization without round-tripping the agent's main loop.
...
type LLMMode int
const (
    ModeHostDelegated LLMMode = iota // Tier 1 — agent-driven
    ModeSampling                     // Tier 2 — client advertised sampling + opt-in
    ModeProvider                     // Tier 3 — server-side provider configured
    ModeDisabled                     // no judgment available → deterministic only
)
```

Resolution rule (per request):
1. If invoked through the Skill or MCP tools by an interactive agent → **HostDelegated**.
2. Else if client advertised `sampling` and `llm.sampling.enabled = true` → **Sampling**.
3. Else if a provider is configured → **Provider**.
4. Else → **Disabled**: search still works; organization is deferred and queued.

> **Never block on judgment.** If no tier is available, organization is *queued*, not failed. Retrieval is always functional (design N4). This is enforced in tests with the `mock` provider and a "no-LLM" client.

---

## 4. MCP tool surface (additions)

The existing data tools (design §13.2: `memory_store`, `memory_search`, `memory_get`, `memory_update`, `memory_delete`, `memory_link`, `memory_list`) are unchanged and need **no LLM**. We add tools that make host delegation first-class.

### 4.1 Read-only / deterministic (no LLM, safe to auto-invoke)

| Tool | Returns | Notes |
|---|---|---|
| `memory_recall` | Top-k memories for a query, with scores | Convenience wrapper over `memory_search` tuned for agent recall (hybrid + recency/importance). |
| `memory_find_duplicate_candidates` | Clusters of near-duplicate chunks (by vector sim ≥ threshold) | Pure vector math. The *decision* to merge is left to the caller. |
| `memory_suggest_organization` | A **work packet**: untagged items, unlinked entities, dup clusters, episodic clusters ready for summarization | The to-do list the host model walks. No LLM server-side. |
| `memory_stats` | Counts, segments, cache hit-rate, pending-organization queue depth | |

### 4.2 Write tools the host model calls back with its results

| Tool | Side effect | Permission class |
|---|---|---|
| `memory_store` | Insert memory/summary | non-destructive |
| `memory_set_tags` / `memory_update` | Update metadata | non-destructive |
| `memory_link` | Add graph edge `(s,p,o)` from extracted relations | non-destructive |
| `memory_merge` | Merge N memories into 1 (host supplies the merged summary); originals tombstoned | **soft-destructive** → requires confirmation flag |
| `memory_forget` | Soft delete (default) / hard delete (explicit) | **destructive** → never auto; explicit only (design N7) |

### 4.3 The autonomous helper (Tier 2/3 only)

| Tool | Behavior |
|---|---|
| `memory_organize` | If mode is Sampling/Provider, runs the organizer jobs end-to-end server-side and returns a report. If mode is HostDelegated, it **does not call an LLM** — it returns the same work packet as `memory_suggest_organization` plus a hint: "no server-side model; drive this via the agent." |

This makes `memory_organize` safe to expose everywhere: it degrades to delegation instead of failing.

---

## 5. End-to-end flows

### 5.1 Recall (no LLM anywhere)
...
sequenceDiagram
    ...
    CC-->>U: synthesized answer grounded in retrieved memories
```

### 5.2 Host-delegated organization (Tier 1 — the common case)
...
sequenceDiagram
    ...
    CC->>SM: memory_merge(ids=[a,b], summary="…model-written…")
    CC->>SM: memory_link(s="cacheImpl", p="supersedes", o="oldImpl")
    CC->>SM: memory_set_tags(id, tags=["caching","decision"])
...
sequenceDiagram
    ...
    SM->>P: extract / summarize / score (server-side LLM)
    P-->>SM: structured JSON
    SM-->>D: report (links added, merges done)
```
...
```toml
[llm]
# Tier ordering is automatic; these gate the optional tiers.
prefer = "host"            # host | sampling | provider  (advisory order start point)

[llm.sampling]
enabled = false            # Tier 2; only used if the client also advertises it
model_hints = ["claude", "gpt"]   # MCP modelPreferences hints
cost_priority = 0.5
intelligence_priority = 0.7

[llm.provider]             # Tier 3 fallback (design §11)
provider = "ollama"
model    = "llama3.1"

[organize]
on_ingest = ["tag", "importance"]   # cheap; delegated or provider
background = ["extract", "dedup", "consolidate"]
...
Using the documented Claude Code skill frontmatter fields:
...
## 8. Codex and other agents (portability)
...
- **Mode matrix**: unit-test all four `LLMMode` paths. A "no-LLM client" fixture must still pass recall and must *queue* (not fail) organization.
...
## 11. Decision log (this addendum)

| # | Decision | Rationale |
|---|---|---|
| H1 | Host-delegated organization is the primary "use host LLM" mechanism | Sampling is unsupported by target clients and deprecating; delegation works today with zero extra cost. |
...
*Open questions: (a) when/if Claude Code or Codex ship sampling, revisit promoting Tier 2 for background autonomy; (b) whether to expose a "review queue" UI for pending merges in the Web UI plugin (design §13.3).*